...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($lead_id > 0 && $message !== '') {
        $stmt = $con->prepare(
            "INSERT INTO revenue_messages (lead_id, message, created_at) VALUES (?, ?, NOW())"
        );
...
if (isset($_GET['lead_id'])) {
    ...
    $stmt = $con->prepare(
        "SELECT message, created_at FROM revenue_messages WHERE lead_id = ? ORDER BY created_at DESC"
    );
